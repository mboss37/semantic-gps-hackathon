...
type ToolLookup = { id: string; name: string; display_name: string | null };
...
export const fetchOrgRoutes = async (
  ...
  const [routesRes, stepsRes] = await Promise.all([
    supabase
      .from('routes')
...
export const fetchRouteDetail = async (
  ...
  const [toolsRes, fallbackRoutesRes] = await Promise.all([
    toolIds.size
      ? supabase.from('tools').select('id, name, display_name').in('id', Array.from(toolIds))
  ...
  for (const t of (toolsRes.data ?? []) as ToolLookup[]) toolById.set(t.id, t);
