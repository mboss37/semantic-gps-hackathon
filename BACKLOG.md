...
- **MCP spec Discussion post (`modelcontextprotocol/modelcontextprotocol` → Ideas - General).** Draft kept private. Frames `_meta.trel` as a working reference implementation under the registered-prefix extension model, six-edge taxonomy (`produces_input_for`, `requires_before`, `suggests_after`, `alternative_to`, `compensated_by`, `fallback_to`) as the proposed shape. Explicitly NOT a SEP, NOT a PR, NOT asking for core schema changes — Discussion only, four open questions for community feedback. Trigger to post: when the empirical traction story is stronger (graph-adherence telemetry shows model-behavior lift on hosted demo + at least one external implementer). Posting now repeats the Discussion #943 failure mode (premature spec signaling without runtime adoption gets philosophical pushback).
