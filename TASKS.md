...
- Demo video recorded: 90s slides (intro + 3 receipts: Replit/Cursor-Supabase/Meta-OpenClaw + 97/88/14 metrics + EU AI Act Aug 2 2026) + ~2min platform tour (Servers → Relationships → Routes → Policies → Playground → Connect). Uploaded YouTube unlisted: https://www.youtube.com/watch?v=fYh2MpMw1ng.
- Submission filed via CV platform: Build-for-what-you-know problem statement, 165-word description from punchlist draft, repo + video links, Opus 4.7 feedback emphasizing model+harness coupling (CLAUDE.md + hooks + rules + subagent worktree isolation), Managed Agents disclosure (not used directly; Semantic GPS positioned as natural complement).
...
- WP-32.4 Discussion post for `modelcontextprotocol/modelcontextprotocol` deferred to BACKLOG P0. Draft is ready (frames `_meta.trel` as a working reference implementation under the registered-prefix extension model, six-edge taxonomy as the proposed pattern, explicitly NOT a SEP). Posting it now is premature — strategic call from Mihael. Goes live after the empirical traction story is stronger.
...
- WP-31.2 Polish bundle, all 5 reviewer-flagged follow-ups in one commit: `oneOf: [route_name | route_id]` constraint on the synthetic tool's `inputSchema` (pushes wire-validation to clients that JSON-Schema-check args), `MAX_TOTAL_CHARS = 1200` rationale comment in `lib/manifest/format-description.ts` (~300 tokens, ~50 KB at 50 tools), `MAX_LISTED_ROUTES = 25` cap with overflow line in the `execute_route` description (bounded for many-route orgs; truncated routes still callable by `route_id`), drift-guard regex hardened in `__tests__/trel-schema.vitest.ts` (strips line comments + requires structural `(?:\|\s*'name'\s*)+` shape so a stray `;` in a comment can't break the union extraction), `.order('to_created_at', desc).limit(10000)` on the `graph_adherence_pairs` SELECT in `app/api/monitoring/graph-adherence/route.ts` (bounded payload + most-recent-wins on truncation). Test mock chain updated to honor the new `.order()` + `.limit()` shape. 434/2/0 tests still green.
...
- WP-30.5 Proto-SEP drafting deferred until WP-30 telemetry shows model-behavior lift on the hosted demo and at least one external org adopts the `_meta.trel` shape. The schema lock at `lib/mcp/trel-schema.ts` is the canonical reference any future SEP submission will cite
