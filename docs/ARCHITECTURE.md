...
| GET | `/api/monitoring?range=…` | Sprint 23: combined volume + per-policy blocks + PII counts in ONE SQL round-trip. Same range vocabulary as `/api/audit`. Auto-picks range on first load (omit `range`). Response: `{volume, blocks, pii, range}` with `dateLabel` pre-formatted. |
