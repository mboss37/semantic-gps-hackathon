...
- **Expose `execute_route` as a standard MCP tool in `tools/list`.** Sprint 30 description enrichment recommends `execute_route('<route_name>')` to the model, but `execute_route` is currently a custom JSON-RPC method on `/api/mcp`, not a tool. Standard MCP clients (Claude Code, Anthropic's `mcp_servers` connector, the SDK) can only call what's in `tools/list` via `tools/call` — so the model sees the recommendation but can't act on it. Empirically caught in the post-merge Claude Code probe (Sprint 30 verification): the agent quoted "prefer execute_route('sales_escalation')" verbatim from descriptions and then asked "but it's not in the tool list — how do I call it?" Fix: register a synthetic `execute_route` tool in `lib/mcp/stateless-server.ts` whose `tools/call` handler dispatches to the existing `executeRoute()` in `lib/mcp/execute-route.ts`. Input schema: `{ route_id?: string, route_name?: string, inputs: object }`. The custom JSON-RPC method stays for backward compat. This closes the loop: enriched description recommends a route, model calls it as a normal tool, gateway runs the saga. ~80 LOC.
