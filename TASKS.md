...
**Sprint 22 — Final demo push (Sat Apr 25 PM):**
> Last-day scope is locked at 3 WPs. Anything outside this list does not ship today.

- ✅ WP-22.1 — Supabase Realtime on `mcp_events`: migration adds table to `supabase_realtime` publication with `REPLICA IDENTITY FULL`; `useRealtimeDashboardEvents` hook subscribes via browser supabase client, forces JWT onto websocket via `setAuth(session.access_token)` (canonical fix — without this RLS sees anon and drops every event). Verified local + hosted. Reviewer findings filed under P2 Hardening. Note: `policy_events` table doesn't exist — policy decisions are JSONB on `mcp_events`, so one publication covers both.
- WP-22.2 — Audit detail Sheet: row-click on `/dashboard/audit` opens a Radix Sheet (right-side drawer) showing full request/response payloads, per-policy verdicts, latency, and Opus thinking trace if present. Files: `app/dashboard/audit/page.tsx` (clickable rows + state), new `components/dashboard/audit-detail-sheet.tsx`, new `app/api/audit/[id]/route.ts` (org-scoped fetch by `mcp_events.id`). Reuse `components/ui/sheet.tsx`. Demo signal: side-slider inspector judges expect from a governance product. ~1h. Hard cap: 1.5h.
- WP-22.3 — NL Policy Author with Opus 4.7: textarea on the policy create/edit flow → "Translate with Opus" button → POST `/api/policies/translate` → Opus 4.7 returns JSON matching a builtin policy schema (cached system prompt of all 12 builtin shapes) → user reviews + saves. Files: new `app/api/policies/translate/route.ts` (Anthropic SDK, `prompt_caching: ephemeral` on system block), update policy create/edit page with NL pane + Sparkles button, validation via existing builtin Zod schemas before save. Sister-project reference (NOT the same DSL — schemas differ): `/Users/mboss37/Projects/semantic-gps/components/policies/nl-translator.tsx` for UX shape only. Demo signal: biggest Opus 4.7 Use lever in the stack — judges see Opus translate plain English to policy JSON live. ~2-3h. Hard cap: 3h.
