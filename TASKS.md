...
**Sprint 23 — Connect UX + Operate-cluster polish (Sat Apr 25 evening, ~6h budget):**
> Today is build polish. Recording is tomorrow. Goal: every dashboard surface reads as ONE product, and a user can actually figure out HOW to call their gateway.

- **WP-23.1 Gateway Connect page** (~2.5–3h) — new `/dashboard/connect` page. Three-tier endpoint cards (org-wide `/api/mcp` + domain-scoped `/api/mcp/domain/<slug>` with selector + server-scoped `/api/mcp/server/<id>` with selector). Token row linking to `/dashboard/tokens` (or showing minted tokens with copy buttons, plaintext NEVER re-shown). Connection snippets per client: curl `tools/list`, Claude Desktop `claude_desktop_config.json` MCP server entry, MCP Inspector launch URL, Anthropic SDK `mcp_servers` block. Optional "Test connection" button firing `tools/list` inline. Sidebar link in Build group right after Tokens. Industry pattern (Kong / MuleSoft / Tyk all have this — "How do I use this?" page is table stakes). Currently the dashboard has zero UX for surfacing the gateway URL — users grep the README. Hard cap: 3h.
- **WP-23.2 Charts → ChartContainer + unified palette** (~2–2.5h) — `components/ui/chart.tsx` is installed but never imported. Wrap all 5 charts (`audit-chart`, `monitoring-{volume,blocks,pii}-chart`, `policy-timeline-chart`) in `<ChartContainer config={...}>` with one palette keyed on event status (`ok`=emerald, `error`=red, `blocked`=amber, `rollback`=orange, `fallback`=blue). Fixes invisible Audit "Events by status" bars (all `fill="hsl(var(--primary))"`), fat-and-lonely Monitoring bars (no `barCategoryGap` / `maxBarSize`), hand-rolled per-chart tooltips. Same primitive overview area chart already uses. Hard cap: 2.5h.
- **WP-23.3 Monitoring KPI hero strip** (~2h, stretch) — 4-card row above charts mirroring overview's `SectionCards`: Total calls / Error rate % / Block rate % / p95 latency, each with delta vs prior period + sparkline. Right now Monitoring opens with 3 mostly-empty stacked bars and zero KPI signal. Only ship if 23.1 + 23.2 are done by 4h in.

Cuts (deferred to BACKLOG): Audit DataTable refactor (3-4h, judges see Sheet not raw rows during demo), gateway token auto-mint (killed — Kong/MuleSoft don't auto-mint; explicit user mint is canonical), hosted E2E smoke test (P1 BACKLOG, useful for tomorrow's pre-recording prep but not today's build polish).
