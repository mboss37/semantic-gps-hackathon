...
**Sprint 25 — Servers UX + Overview cleanup (Sat Apr 25 late night, ~3.5h budget):**
> Last UX polish slot before recording. Servers pages are the most-touched build surface — cards lack health/traffic signal, the detail page is bottom-heavy with empty cards, and `server-config-snippet.tsx` still leaks the same dead tunnel URL Connect just fixed. Overview also duplicates audit and uses inconsistent timerange vocab vs Monitoring. P0 bug + P1 polish bundle.

- **WP-25.1 server-config-snippet origin fix** (~10m, P0 bug) — same `useSyncExternalStore(window.location.origin)` pattern that landed in Connect (`6ccfa89`). Kills the cloudflared tunnel URL leak in the per-server MCP client config block.
- **WP-25.3 backend: serverId filter** (~30m) — `fetchMonitoringWindowed` accepts optional `serverId` param; `/api/monitoring` accepts `?serverId=`. Existing /dashboard/monitoring callers (no serverId) keep current behavior. Unblocks the scoped KPI strip on server detail.
- **WP-25.2 Server card overhaul** (~1h) — health dot top-left, 24h traffic stat replacing mystery date, overflow `⋯` menu (Delete + Rediscover), capped tool pills. Whole card clickable. Page fetches per-server traffic counts via single batched GROUP BY on `mcp_events`.
- **WP-25.3 frontend: server detail reflow** (~1h) — header stat row (tools / 24h calls / error rate / registered), Origin status lifted to top, scoped `<MonitoringKpiStrip>` filtered by serverId, Resources/Prompts cards collapsed into one inline row (or hidden when both zero).
- **WP-25.4 Overview cleanup** (~1h) — drop bottom DataTable (audit duplicate), `<SectionCards>` rewritten with current+prior delta pattern (mirroring Monitoring KPI strip), Gateway Traffic chart shares Monitoring's timerange vocabulary (default 24h instead of months/days).
