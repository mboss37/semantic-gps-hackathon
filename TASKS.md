...
**Sprint 29: Submission day (Sun Apr 27):**
- Hosted demo org seeded via Supabase MCP: 3 servers + 14 tools + 13 relationships + 2 routes (sales_escalation + cross_domain_escalation) + 4 policies (business_hours/write_freeze/redact_pii/injection_guard) + gateway token. 17 audit events populated (mix of allowed/blocked_by_policy/origin_error) for dashboard demo realism.
- Demo video recorded: 90s slides (intro + 3 receipts: Replit/Cursor-Supabase/Meta-OpenClaw + 97/88/14 metrics + EU AI Act Aug 2 2026) + ~2min platform tour (Servers → Relationships → Routes → Policies → Playground → Connect). Uploaded YouTube unlisted.
- Submission filed via CV platform: Build-for-what-you-know problem statement, 165-word description, repo + video links, Opus 4.7 feedback emphasizing model+harness coupling, Managed Agents disclosure (not used directly; positioned as natural complement).
- Repo cleanup post-submission: Slides-Brief.md + Semantic GPS Intro Deck.html + SUBMISSION-PUNCHLIST.md removed from root. BACKLOG.md P0 hackathon-deadline section cleared.

**Hotfix `45bf6e0`: spec-compliant MCP HTTP-Streamable session handshake (post-Sprint-29):**
- Mule Anypoint MCP registration failed with `"origin response is not valid MCP tools/list"`: spec-strict servers reject methods before `initialize` with HTTP 200 + empty SSE body.
- New `lib/mcp/handshake.ts::runMcpHandshake`: per-request `initialize → mcp-session-id → notifications/initialized` with graceful no-op on permissive servers. Wired into `discover-tools.ts` + `proxy-http.ts`. Verified against 5 servers (Demo SF/Slack/GitHub permissive, Hyperterminal permissive, Mule strict). 8 focused unit tests.

**Sprint 30: TRel description-enrichment + protocol stake (Mon Apr 27):**
- VISION.md "The protocol play" section declares `_meta.trel` standardization goal pre-sprint (255e522). Engagement plan: comment on Discussion #943 → exploratory MCP Discussion after telemetry → SEP after 3+ implementers.
- WP-30.1+30.2 `lib/manifest/format-description.ts` + `lib/mcp/stateless-server.ts` wiring: folds relationship graph into standard `description` field (every MCP client preserves it; `_meta` empirically stripped consumer-side). Per-scope edge caps + 1200-char total cap + `display_description` override.
- WP-30.3 `lib/mcp/trel-schema.ts` schema lock: 6-edge wire taxonomy (`TREL_EDGE_TYPES`) + 2 linter-only types (`LINTER_ONLY_EDGE_TYPES`) kept off the wire. Producer filters by `TREL_EDGE_TYPES.includes(...)`. Drift-guard test reads source, regex-extracts union, asserts every member is classified.
- WP-30.4 `graph_adherence_pairs` SQL view (with `security_invoker=on` per reviewer catch — Postgres views bypass RLS by default) + `GET /api/monitoring/graph-adherence?range=...` endpoint partitioning adherence rate by `governed=true|false`. Returns `rate: null` on empty buckets. Sprint 30 hard-won lessons #48 + #49.

**Sprint 31: Close the TRel loop — execute_route as a tools/list tool (Mon Apr 27):**
- WP-31.1 Synthetic `execute_route` tool surfaced on `tools/list` (governed only) via new `lib/mcp/execute-route-tool.ts`. `tools/call execute_route` short-circuits to existing `executeRoute()` runtime with same `policyCtxBuilder`/`traceId`/`organizationId` threading. Native JSON-RPC method stays for backward compat. Closes the agent-recommendation loop after Sprint 30 enrichment recommends it. Empirically caught in post-merge Claude Code probe.
- WP-31.2 Polish bundle: `oneOf` constraint on `inputSchema`, `MAX_LISTED_ROUTES = 25` cap with overflow line, `MAX_TOTAL_CHARS = 1200` rationale comment, drift-guard regex hardened (line-comment strip + structural pattern), `.order().limit(10000)` on graph_adherence_pairs SELECT. Hard-won lesson #51.

**Sprint 32: Credibility — gap-acknowledgment + lean security policy (Mon Apr 27):**
- WP-32.1 `docs/VISION.md` `## Known gaps and what's next` section: 4 lean subsections (Performance, Operational observability via OTel + W3C `traceparent`, Threat model, Failure modes / chaos via Temporal-grade validation). Names the gap + canonical pattern; no commitment dates.
- WP-32.2 Initially shipped a full `SECURITY.md` (90-day SLA, scope tables, hall-of-fame) + `docs/THREAT-MODEL.md` (5 trust boundaries + 10-row attack-surface matrix). Scaled back to MVP-appropriate: lean 6-line `SECURITY.md` (email channel + best-effort response, no SLA promise), formal threat-model document moved to `_private/THREAT-MODEL-draft.md` until customers exist who'd benefit from public disclosure. The internal thinking is preserved; the public surface is right-sized.
- WP-32.3 Light audit pass mapped existing controls (SSRF, encryption, RLS, security_invoker, redactPayload, append-only audit) to internal threat-model rows. No code changes.
- WP-32.4 Public MCP Discussion post deferred to BACKLOG P0. Draft + proto-SEP both live in repo-local gitignored `_private/` dir per new convention.
...
## Session Log
- 2026-04-27: Wrap-up after Sprints 30 + 31 + 32 + handshake hotfix. Three sprints landed on main since hackathon submission close. Sprint 30 (TRel description enrichment + `_meta.trel` schema lock + graph-adherence metric) converted latent graph into model-visible behavior — empirical proof via fresh Claude Code probe (model went from inventing edges to reading declared compensators/fallbacks/join-keys). Sprint 31 (synthetic `execute_route` tool + polish bundle) closed the agent-recommendation loop. Sprint 32 (credibility docs: VISION gaps + lean SECURITY.md, with the formal threat-model artifact kept in `_private/` until customers exist) addressed external feedback after a calibration pass — initial shipment was over-engineered for an 0-customer MVP. Hotfix `45bf6e0` shipped spec-compliant HTTP-Streamable handshake for Mule-class strict servers. Hard-won lessons #48-51 documented. `_private/` gitignored convention introduced for non-public drafts.
- 2026-04-27: Hackathon shipped. Hosted demo seeded (3 MCPs + 14 tools + 2 routes + 4 policies + 17 audit events). Demo video recorded + uploaded. Submission filed via CV platform. Root cleanup. BACKLOG.md P0 section cleared.
- 2026-04-26: Sprint 28 shipped on `feat/route-authoring` then merged to main (--no-ff at `fb3778c`). 6 WPs: POST/DELETE /api/routes + import dialog + Copy as JSON + design-review fixes. JSON-import path chosen as MVP de-risk; v2 visual editor in BACKLOG. 942 insertions, 356/2/0 tests.
