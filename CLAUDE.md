...
- **Capture bag unwraps MCP envelope**: in-process HTTP-Streamable MCPs return `{content:[{type:"text", text:"<JSON>"}]}`; `proxy-http.ts::extractResult` strips the outer wrapper leaving a bare array in `postResult`. `lib/mcp/route-utils.ts::unwrapMcpEnvelope()` at capture time so `input_mapping` and `rollback_input_mapping` DSL (`$steps.<key>.result.<field>`) resolves against the logical object, not the envelope. Handles both bare-array and wrapped shapes defensively; OpenAPI bodies pass through. Shipped Sprint 19 WP-19.9.
- **`requireAuth` is React-`cache()`-wrapped**: per-request dedup of Supabase `getUser()` + `getSession()` round-trips. Layout + page + helpers within one RSC render share one auth call (3-4 network hops → 1, ~1s saved per dashboard navigation). Server Actions invoked from the same render share the cache too. Cache scope is per-request; isolated requests re-execute. No-op outside RSC render context (vitest), so unit-testing the dedup itself isn't possible: contract tests on wrap presence + invocability are the regression guard. Shipped Sprint 20 WP-20.2.
- **Onboarding action forces JWT refresh**: `auth.updateUser({data})` does NOT refresh the access token, so the cookie keeps stale claims (signup-era `profile_completed:false`) until natural expiry. `app/onboarding/actions.ts` calls `auth.refreshSession()` after the `memberships.profile_completed=true` flip so the JWT hook re-stamps and `proxy.ts` reads `profile_completed:true` on the next `/dashboard` hit. Without this, fresh signups bounce back to `/onboarding` indefinitely. Shipped Sprint 20 WP-20.1.
- **Dashboard chrome split: muted main, pure-black chrome**: `--background: oklch(0.10 0 0)` lifts the main content card off pure black; `--sidebar: oklch(0 0 0)` keeps sidebar + rail + header pure Vercel black. Inverts shadcn defaults to favor chrome-as-frame, content-as-stage (Linear/Vercel/Stripe pattern). `SidebarInset` gets `border overflow-hidden` so the rounded-xl shape clips the `bg-sidebar` header strip and the 1px outline reveals the inset card edges. Shipped Sprint 21 chrome polish.
- **Dashboard auto-refresh on tab return**: `useDashboardRefresh()` in `hooks/use-dashboard-refresh.ts` fires `router.refresh()` (RSC tree reload) AND a `'semgps:dashboard-refresh'` window CustomEvent (for client-state components like the traffic chart that hold their own `useState`/`useEffect` fetches). 2s debounce absorbs `visibilitychange` + `focus` double-fire. SiteHeader runs the listeners + a manual refresh button. Realtime subscription path deferred to BACKLOG P1. Shipped Sprint 21 WP-21.5.
- **Supabase Realtime needs explicit `setAuth` + `REPLICA IDENTITY FULL` on RLS tables**: `@supabase/ssr`'s cookie hydration carries the JWT for HTTP requests but NOT for the realtime websocket. Channels subscribe with whatever auth was set at client init (often empty); RLS evaluates as anon and drops every event silently. Pattern: `auth.getSession()` → `realtime.setAuth(session.access_token)` → `channel.subscribe()`, mounted once at the dashboard shell. Plus `ALTER TABLE ... REPLICA IDENTITY FULL` so the replication stream carries the columns RLS filters on (DEFAULT replica identity sends only the PK). `useRealtimeDashboardEvents` in `hooks/use-realtime-dashboard-events.ts`. Shipped Sprint 22 WP-22.1.
- **Saga compensator follows the fallback target, not the primary**: when a step's `fallback_to` succeeds, the captured result + compensator semantics belong to the FALLBACK tool. Picking primary's `compensated_by` orphans fallback artifacts on rollback. Fix: `ExecuteRouteFallbackUsed` carries `fallback_tool_id` so `executeRollback` picks the right compensator edge; `route_steps.fallback_rollback_input_mapping jsonb` translates the fallback target's captured result into ITS OWN compensator's input shape (mirror of `rollback_input_mapping`). Mapping precedence: `fallback_rollback_input_mapping` (when fallback was used) → `rollback_input_mapping` → producer's result verbatim. E2E proven: zero orphans on saga halt after fallback succeeds. Shipped Sprint 22 WP-22.4 follow-on.
- **Charts speak one palette + one time-range vocabulary**: `lib/charts/palette.ts` is the single source of truth for status colors / labels / Tailwind badge classes / shadcn `ChartConfig` builders. Audit table, audit timeline LineChart, audit detail Sheet, monitoring 3 charts, policy timeline all read one map. Adding a status (e.g. `rollback_executed` orange) is a one-line edit. `lib/monitoring/range.ts` owns the time-range vocabulary (`'15m' | '30m' | '1h' | '6h' | '24h' | '7d'` with bucket spec each, ~30 buckets/range target): Monitoring + Audit share it so users learn the picker once. Server pre-formats `dateLabel` so charts stay dumb. Shipped Sprint 23.
- **Audit + Monitoring auto-pick range on first load**: server queries the latest event's `created_at`, picks the smallest range whose window contains it via `pickAutoRange()`. Lands users on a non-empty chart instead of an empty 1h default when their last activity rolled outside the window. User picks override + are sticky. Client passes no `range` param on initial fetch; syncs `range` state from response. Datadog/Honeycomb pattern. Shipped Sprint 23 (`/api/audit` + `/api/monitoring` share the helper).
- **Audit page = forensic search-refinement UX, not status-count summary**: Datadog/Splunk/Sumo/Honeycomb shape. Timeline LineChart (events over time, one line per status) at top + filter pills + result table. Status-count bar charts duplicate the filter pills → killed. Filter pills affect chart AND table. Time-range picker shares Monitoring vocabulary. Old `audit-row.tsx` button-list replaced with shadcn `<Table>`. Shipped Sprint 23.
- **User-copyable URL surfaces use `useSyncExternalStore(window.location.origin)`, never `NEXT_PUBLIC_APP_URL`**: env var was a stale-tunnel footgun on local dev (cloudflared URLs leaked into Connect snippets + server-config-snippet long after the tunnel died). Pattern: `subscribeNoop` no-op subscriber + getSnapshot reads `window.location.origin`, server snapshot returns `''` for hydration-safe SSR. Always reflects the host the user is actually viewing the page from (localhost / Vercel / tunnel). Shipped Sprint 24-25 (`6ccfa89` + Sprint 25 server-config-snippet).
- **SiteHeader is pure chrome; each page owns its `<h1>`**: sidebar trigger + brand cluster ("Built with Opus 4.7" pill + org name) + refresh button only. Removed the page-title `<h1>` after the duplicate-title regression user flagged across all 11 dashboard pages. Pattern matches Linear/Vercel/Stripe: chrome contains no page-specific labels. When adding a new dashboard page, ensure main content has its own h1; don't expect SiteHeader to label it. Shipped Sprint 24.
- **Server cards derive health from event status mix, not live probes**: single batched `mcp_events` GROUP BY query on the servers list page returns calls + error counts per server for the last 24h. Health = `unknown` (0 calls) | `down` (errors >= calls) | `degraded` (some errors) | `ok` (none). Cheaper than N parallel `/api/servers/[id]/health` probes on render; stale by ~refresh-interval is fine for the list view. Live probe stays available on detail page where health-now matters. Shipped Sprint 25.
- **Demos beat perfect code**: if any of the 4 success criteria breaks on stage, fall back to a recorded clip and ship
- **TRel ships as `_meta.trel.*` registered-prefix MCP extension**: 6-edge wire taxonomy (`produces_input_for` / `requires_before` / `suggests_after` / `alternative_to` / `compensated_by` / `fallback_to`) locked at `lib/mcp/trel-schema.ts`; 2 linter-only types (`mutually_exclusive` / `validates`) deliberately kept off the wire. Producer in `lib/mcp/stateless-server.ts` filters by `TREL_EDGE_TYPES.includes(...)` so linter types can never leak. Drift-guard test reads `lib/manifest/cache.ts` source, regex-extracts the union, asserts every member is classified. Schema is the future SEP normative reference; engagement plan lives in `docs/VISION.md` "The protocol play". Shipped Sprint 30.
- **Description-as-graph activation**: standard MCP clients (Claude Desktop, Cursor, Anthropic `mcp_servers` Beta connector) strip `_meta` before reaching the model, empirically verified. The standard `description` string is the only field with provably 100% client coverage. `lib/manifest/format-description.ts` folds the relationship graph into descriptions with type-prefixed lines (`Rollback:`, `Fallback:`, `After this:`, `Before this:`), per-scope edge caps (org=3, domain=5, server=8), 1200-char total cap. `display_description` manual override always wins. `_meta.trel` still emitted for future-proofing. Shipped Sprint 30 WP-30.1+30.2.
- **`execute_route` exposed as synthetic `tools/list` tool**: runtime-injected from `manifest.routes` data when `governed && manifest.routes.length > 0`; `tools/call` short-circuits to the existing `executeRoute()` runtime via `lib/mcp/execute-route-tool.ts`. Description lists every available route by name + step count + rollback/fallback guards (capped at 25 routes); `inputSchema` declares `oneOf: [{required:['route_name']},{required:['route_id']}]`. Native `ExecuteRouteRequestSchema` JSON-RPC method stays for backward compat. Shipped Sprint 31 WP-31.1 — closes the agent-recommendation loop the description enrichment opens.
- **MCP HTTP-Streamable session handshake required**: `lib/mcp/handshake.ts` runs `initialize → mcp-session-id → notifications/initialized` per request. Required for spec-strict servers (MuleSoft Anypoint, anything on the SDK session middleware) which reject any method before `initialize` with HTTP 200 + empty SSE body. Permissive servers (in-process vendors, Hyperterminal) gracefully no-op. Wired into `discover-tools.ts` (registration) + `proxy-http.ts` (`tools/call` proxy). Stateless (no caching). Shipped hotfix `45bf6e0`.
- **Postgres views need `security_invoker = on` for RLS to flow through**: views run with the OWNER's privileges by default and bypass RLS on base tables. Caught by reviewer in Sprint 30 WP-30.4 graph-adherence migration. Every view scoped by `organization_id` must `ALTER VIEW <name> SET (security_invoker = on);` after the `CREATE OR REPLACE VIEW`. Otherwise authenticated callers see all rows across all tenants. Audit rule: any new migration adding a view that joins to RLS-protected tables MUST ship `security_invoker=on`.
