...
| GET | `/api/monitoring/graph-adherence?range=…` | Sprint 30 WP-30.4: graph adherence rate — fraction of consecutive `tools/call` pairs (within one `trace_id`) that follow a known TRel edge in the org's `relationships` table. Partitioned by `governed=true|false` so `/api/mcp` (governed) and `/api/mcp/raw` (ungoverned) buckets contrast. Returns `rate: null` (not 0) on empty buckets to distinguish "no data" from "zero adherence." Backed by `graph_adherence_pairs` view with `security_invoker=on` (RLS scoped per caller). Same range vocabulary as `/api/audit` + `/api/monitoring`. |
...
48. **Postgres views need `security_invoker = on` for RLS to flow through.** Postgres views run with the OWNER's privileges by default (the migration applier — `postgres` superuser on Supabase) and bypass RLS on base tables. Caught by reviewer in Sprint 30 WP-30.4 graph-adherence migration: original `20260427030000_graph_adherence_view.sql` had a misleading comment claiming "views inherit RLS from base tables" — false; any authenticated caller would have seen ALL rows across all tenants. Fix is one line: `ALTER VIEW <name> SET (security_invoker = on);` after the `CREATE OR REPLACE VIEW`. Then RLS policies on `mcp_events` + `tools` + `servers` (or whatever's joined) evaluate against the calling user's JWT, not the view owner's superuser context. Audit rule: any new migration adding a view that joins to RLS-protected tables MUST ship `security_invoker=on`. Reviewer should grep `CREATE.*VIEW` and confirm.
49. **Description-as-graph is the only universal TRel activation lever.** Sprint 30 verified empirically: every consumer-side MCP path strips `_meta` before reaching the model. Probe through Anthropic `mcp_servers` Beta connector → "no `_meta` field, no `annotations` field, no nextTool / chain / workflow hints." Fresh `claude mcp add` session asking the agent to describe a tool → "inferring from names + typical CRM/ops patterns," invented cross-MCP edges that don't exist, missed every compensator and fallback. The standard `description` string is the only field with provably 100% client coverage. `lib/manifest/format-description.ts` folds the relationship graph into descriptions with type-prefixed lines (`Rollback:`, `Fallback:`, `After this:`, `Before this:`), per-scope edge caps (org=3, domain=5, server=8), and a 1200-char total cap. `display_description` manual override always wins. `_meta.trel` still emitted for clients that DO start parsing it (future-proofing). Post-merge probe confirmed model now reads declared edges verbatim.
50. **MCP HTTP-Streamable session handshake is required for spec-strict servers.** Spec-compliant servers (MuleSoft Anypoint, anything built on the SDK's session middleware) reject ANY method before `initialize` with HTTP 200 + an empty SSE body. Our discovery + proxy code originally posted `tools/list` / `tools/call` directly: worked against permissive servers (in-process vendor MCPs, Hyperterminal) but failed silently against strict ones with `"origin response is not valid MCP tools/list"`. Canonical fix: per-request handshake in `lib/mcp/handshake.ts::runMcpHandshake` — POST `initialize` with `protocolVersion: '2025-03-26'` + capabilities + clientInfo → capture `mcp-session-id` response header → POST `notifications/initialized` (spec-mandated; some strict servers wait for it before serving any other method) → carry `Mcp-Session-Id: <id>` on every subsequent call. Stateless (no caching across requests; Vercel-friendly). On any failure (HTTP 5xx, JSON-RPC -32601 method-not-found on init, network drop) returns `sessionId: null` so callers fall through to direct calls. SSRF errors propagate. Wired into `discover-tools.ts` (registration path) and `proxy-http.ts` (tools/call proxy). Verified against 5 production servers including Mule ERP. Shipped hotfix `45bf6e0`.
51. **Synthetic MCP tool at request time — agent-callable shim over JSON-RPC primitives.** Pattern for exposing system primitives (orchestration methods, planning helpers) as standard MCP tools so any compliant MCP client can invoke them through `tools/call`. Without this, custom JSON-RPC methods are unreachable from Claude Code, Cursor, Anthropic's connector, etc.: they only call what's in `tools/list`. Build the descriptor from manifest data at request time (not from a DB row): pure helper `buildXxxToolDescriptor(manifestData) → {name, description, inputSchema} | null` returns null when there's nothing to expose. In `tools/list`, after the catalog `.map(...)`, conditionally `tools.push(synthetic)` when the predicate holds (`governed && manifest.routes.length > 0`). In `tools/call`, short-circuit on the tool name BEFORE catalog lookup: resolve params (e.g. friendly `route_name` → DB UUID `route_id`), dispatch to the existing JSON-RPC runtime with the same `policyCtxBuilder` / `traceId` / `organizationId` threading. Native JSON-RPC method stays for backward compat. Audit shape: top-level `tools/call` row uses `method: 'tools/call'` + `tool_name: 'execute_route'`. Per-step rows inside the runtime use `method: 'execute_route.step'`. Both correlate via `trace_id`. Schema: declare `oneOf: [{required:['route_name']},{required:['route_id']}]` so JSON-Schema-checking clients catch malformed args before dispatch. Shipped Sprint 31 WP-31.1 in `lib/mcp/execute-route-tool.ts`. Same pattern reusable for `discover_relationships` / `find_workflow_path` / `validate_workflow` / `evaluate_goal` if they need agent-callable surface.
